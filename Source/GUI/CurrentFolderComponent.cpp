...
void CurrentFolderComponent::init (juce::ValueTree rootPropertiesVT)
{
    ...
    // the folder being viewed and the file last opened change independently
...
{
    // The open project is a HIHAT.INI, and the folder being viewed is the one it
    // sits in. Showing the file's own path keeps the two from disagreeing, and
    // the file is the last crumb, so it is the one picked out.
    const auto openFile { appProperties.getRecentlyUsedFile (0) };
    pathEndsInFile = openFile.isNotEmpty ();
    const auto path { pathEndsInFile ? openFile : appProperties.getMostRecentFolder () };

    pathSegments.clear ();
    pathSegments.addTokens (path, juce::File::getSeparatorString (), {});
...
{
    g.fillAll (findColour (ClutchColours::listBackground));
    g.setColour (findColour (ClutchColours::outline));
    g.drawHorizontalLine (getHeight () - 1, 0.0f, static_cast<float> (getWidth ()));

    // breadcrumbs: the folders are context, so they are dimmed; the open file is
    // the part that matters, so it is not
    const auto separator { juce::String (juce::CharPointer_UTF8 ("\xe2\x80\xba")) };
    constexpr auto kGap { 5 };
    const auto contextFont { ClutchType::body () };
    const auto currentFont { ClutchType::bodyStrong () };
    ...
    for (auto segmentIndex { 0 }; segmentIndex < pathSegments.size (); ++segmentIndex)
    {
        ...
        const auto isFile { isLast && pathEndsInFile };
        const auto segment { pathSegments [segmentIndex] };
        const auto& font { isFile ? currentFont : contextFont };
        const auto segmentWidth { ClutchPaint::textWidth (font, segment) };
        if (x + segmentWidth > rightEdge)
            break;

        g.setFont (font);
        g.setColour (findColour (isFile ? ClutchColours::text : ClutchColours::textDim));
        g.drawText (segment, textArea.withX (x).withWidth (segmentWidth + 1), juce::Justification::centredLeft, false);
        x += segmentWidth + kGap;

        if (! isLast)
        {
            const auto separatorWidth { ClutchPaint::textWidth (contextFont, separator) };
