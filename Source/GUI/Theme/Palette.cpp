...
namespace
{
    ...
    // Near-black is violet biased rather than neutral grey; the accent is a cyan,
    // and the sample slots borrow the module's orange for what is sounding.
    ...
    const juce::Colour kLedDark { 0xff3ad17f }, kLedLight { 0xff12925a };
    const juce::Colour kLedEdgeDark { 0xff7df0b6 }, kLedEdgeLight { 0xff0a6b41 };
    const juce::Colour kAuditionDark { 0xffe09324 }, kAuditionLight { 0xffa8600a };

    // the slide switch track oolib asks for, and the rim of an accent fill
...
Palette::Palette ()
{
    ...
    specs =
    {
        ...
        { ClutchColours::windowBackground, kInk900Dark,  kInk900Light },
        { ClutchColours::fieldBackground,  kInk950Dark,  kInk950Light },
        { ClutchColours::listBackground,   kInk850Dark,  kInk850Light },
        { ClutchColours::panelHeader,      kInk800Dark,  kInk800Light },
        { ClutchColours::buttonBackground, kInk750Dark,  kInk750Light },
        { ClutchColours::tabBackground,    kInk850Dark,  kInk850Light },
        { ClutchColours::dialogBackground, kInk900Dark,  kInk900Light },
        { ClutchColours::outline,          kLineDark,    kLineLight },
        { ClutchColours::outlineDim,       kLineSoftDark, kLineSoftLight },
        { ClutchColours::outlineStrong,    kInk650Dark,  kInk650Light },
        { ClutchColours::accent,           kAccentDark,  kAccentLight },
        { ClutchColours::accentDeep,       kAccentDeepDark, kAccentDeepLight },
        { ClutchColours::accentEdge,       kAccentEdgeDark, kAccentEdgeLight },
        { ClutchColours::selectedRow,      kSelectDark,  kSelectLight },
        // a halo is a smudge on a light background, so this fades out rather than flips
        { ClutchColours::accentGlow,       kAccentDark.withAlpha (0.5f), transparent },
        { ClutchColours::ledOn,            kLedDark,     kLedLight },
        { ClutchColours::ledEdge,          kLedEdgeDark, kLedEdgeLight },
        // a lamp glows into a dark menu; on a light one it would only be a smudge
        { ClutchColours::ledGlow,          kLedDark.withAlpha (0.55f), transparent },

        // ---- ink on the panels ----
        { ClutchColours::text,           kTextDark,       kTextLight,       S::background, 1.00f },
        { ClutchColours::textDim,        kDimDark,        kDimLight,        S::background, 0.80f },
        { ClutchColours::menuHeaderText, kMuteDark,       kMuteLight,       S::background, 0.80f },
        { ClutchColours::textGhost,      kGhostDark,      kGhostLight,      S::background, 0.40f },
        { ClutchColours::accentText,     kAccentTextDark, kAccentTextLight, S::background, 0.60f },
        { ClutchColours::audition,       kAuditionDark,   kAuditionLight,   S::background, 0.30f },
        { ClutchColours::dropTarget,     kGreenDark,      kGreenLight,      S::background, 0.35f },

        // ---- ink on an accent fill ----
        { ClutchColours::accentInk, kAccentInkDark, kAccentInkLight, S::accentFill, 1.00f },
...
void Palette::resolve ()
{
    ...
    const auto backgroundColour { surfaceAt (ClutchColours::windowBackground, background) };
    const auto accentColour { surfaceAt (ClutchColours::accent, background) };

    std::vector<float> luminance (3, 0.0f), luminanceAtWhite (3, 0.0f);
    ...
    setSurface (Surface::background, backgroundColour, surfaceAt (ClutchColours::windowBackground, 1.0f));
    setSurface (Surface::accentFill, accentColour, surfaceAt (ClutchColours::accent, 1.0f));
