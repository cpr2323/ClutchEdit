...
/*
    ...
    The switch is per-surface, not global, so each flip token names the surface it
    actually sits on rather than assuming the window background.
*/
class Palette
{
public:
    ...
    // Every pair this palette defines, for ClutchLookAndFeel to install. Includes
    // the ClutchColours IDs, the JUCE widget IDs the app relies on, and oolib's.
...
private:
    ...
    enum class Surface { none, background, accentFill };
